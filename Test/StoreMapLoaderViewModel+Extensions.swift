//
//  StoreMapLoaderViewModel+Extension.swift
//  compass_sdk_ios
//
...
extension StoreMapLoaderViewModel {
    ...
    func handle(_ message: MessageResponse, completion: ((MessageResponse) -> Void)? = nil) {
        ...
        case .mapData(let mapData):
            ...
            self.statusService.emitPointOfInterestEvent(PointOfInterest: mapData.pointsOfInterest)
        ...
        case let .customRouteError(customRouteError):
            Log.debug("customRouteError with message: \(customRouteError)")
            let selectedPinLocation = currentPins.filter { $0.selected }.map {
                "\($0.zone ?? "").\($0.aisle ?? 0).\($0.section ?? 0)"
            }.joined(separator: ",")
            let unselectedPinLocation = currentPins.filter { !$0.selected && $0.type != .campaign }.map {
                "\($0.zone ?? "").\($0.aisle ?? 0).\($0.section ?? 0)"
            }.joined(separator: ",")
            let outOfBoundsCoords = customRouteError.outOfBoundPath.map { OutOfBoundsCoord(x: Int($0.x), y: Int($0.y)) }
            let payload = NavigationErrorAnalytics(
                selectedPinLocation: selectedPinLocation,
                unselectedPinLocation: unselectedPinLocation,
                outOfBoundsCoords: outOfBoundsCoords,
                type: "custom_route",
                isStaticPath: isStaticPathVisible
            )
            Analytics.navigationError(payload: payload)
        default:
    ...
    func updateNavigationServicesIfNeeded(for pinRenderedMessage: PinRenderedMessage) {
        ...
        if hasValidPin {
            ...
            if wasUserNavigatingBefore {
                logNavigationEndEvent() // Log navigation end event for earlier pin
            }

            if let pin = pinRenderedMessage.pins?.first( where:{ $0.selected && $0.errorData == nil }) {
                logNavigationStartEvent(pin: pin)
            }
        } else {
    ...
    func renderPinsIfNeeded(floorCount: Int) {
        defer {
            hasLoadedMapView.send(true)
            mapFocusManager.isMapViewPresent.send(true)
        }
        guard !hasLoadedMapView.value else { return }
    ...
    func handleAsset(waspPosition: CGPoint) {
        ...
        guard let assetId = (userDefaults.object(forKey: UserDefaultsKey.uuidList.rawValue) as? [String])?.first else {
