//
//  MockStoreMapLoaderViewModel.swift
//  compass_sdk_iosTests
//
//  Created by Young Jin Ju on 5/21/24.
//
...
class MockStoreMapLoaderViewModel {
    ...
    var floorSelected = 0
    var mockPinForGetUserDistance: Pin?
    
    // Zoom action tracking
    var onStoreMapZoomChangeCallCount = 0
    var lastZoomType: ZoomActionType?
    
    // Floor action tracking
    var onStoreMapFloorChangeCallCount = 0
    var lastFloorLevelType: FloorLevelType?
    
    // Test helper properties
    var _navigationSessionState: NavigationSessionState?
    var _navigationEnabled: Bool?
    var _requestHandler: ((MessageRequest) -> Void)?
    var _updateNavigationRouteHandler: (([Point], Int, PinList?, RenderPinsRequest?) -> Void)?
    var routeCancellable: AnyCancellable?
    var isStaticPathVisible = false
    var staticPathPreviewCancellable: AnyCancellable?
    var pinLocationFetchCompletionHandler: ((PinRenderedMessage) -> Void)?
    var isPinDistanceFetched = false
    var _assetService: AssetService { assetService }
    var staticPathPreviewService: StaticPathPreviewService { mockServiceLocator.getStaticPathPreviewService() }
    private var mockServiceLocator: ServiceLocatorType

    init(blueDotMode: BlueDotMode, serviceLocator: ServiceLocatorType) {
        self.mockServiceLocator = serviceLocator
    ...
    func _resetMock() {
        ...
        mockPinForGetUserDistance = nil
        onStoreMapZoomChangeCallCount = 0
        lastZoomType = nil
        onStoreMapFloorChangeCallCount = 0
        lastFloorLevelType = nil
    }
    ...
    func onStoreMapZoomChange(zoomType: ZoomActionType, _ completion: (() -> Void)?) {
        onStoreMapZoomChangeCallCount += 1
    ...
    func onStoreMapFloorChange(levelType: FloorLevelType, _ completion: (() -> Void)?) {
        onStoreMapFloorChangeCallCount += 1
        lastFloorLevelType = levelType
        guard let webView = mapViewDelegate?.webView else { return }
...
extension MockStoreMapLoaderViewModel: StoreMapViewActionable {
    ...
    func getUserDistance(_ request: GetPinLocationRequest, completion: ((PinRenderedMessage) -> Void)?) {
        _getUserDistanceCalled = true
        self.pinLocationFetchCompletionHandler = completion
        self.isPinDistanceFetched = true
        let pinsToReturn = mockPinForGetUserDistance != nil ? [mockPinForGetUserDistance!] : [Pin(type:.aisleSection)]
        completion?(PinRenderedMessage(pins: pinsToReturn, topLeft: nil, bottomRight: nil))
        
        // Call test handler if provided
        _requestHandler?(.pinLocationRequested(request))
    }
    ...
    func canDisplayNavigationButton() -> Bool {
        // Use test state if provided, otherwise return default
        return _navigationSessionState?.hasPinsOnMap ?? false
    }

    func getMapURLRequest() -> URLRequest? {
        guard let url = URL(string: "https://www.walmart.com/storemap?mapType=test&compressionEnabled=true&cdnEnabled=true") else {
            return nil
        }
        var request = URLRequest(url: url)
        request.setValue("test-value", forHTTPHeaderField: "test-header")
        return request
    }
    
    func viewDidAppear() {
        guard blueDotMode != .none, isMapViewLoaded else { return }
...
private extension MockStoreMapLoaderViewModel {
    ...
    func renderPinsIfNeeded(floorCount: Int) {
        ...
        guard !isMapViewLoaded else { return }
