//
//  MockCompassViewModel.swift
//  compass_sdk_iosTests
//
//  Created by Young Jin Ju on 4/8/24.
//
...
final class MockCompassViewModel: CompassViewModelType {
    var storeMapViewController: UIViewController? = UIViewController()
    ...
    func getAccessToken(authParameter: compass_sdk_ios.AuthParameter) -> AnyPublisher<compass_sdk_ios.AccessTokenResponse?, any Error> {
        ...
        if authParameter.tokenType != TokenType.User.rawValue {
            ...
            let response = AccessTokenResponse(accessToken: authParameter.clientSecret,
    ...
    func updateEvent(compassEvent: CompassEvent) {
        ...
        serviceLocator.getEventService()
            .cacheAndBootstrapAsset(compassEvent, position: position.convertToCompass(using: converter).asCGPoint(), onCompletion: { _ in })
